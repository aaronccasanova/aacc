...
interface FretboardProps {
  ...
  // onChange?: (state: FretboardState) => void
}
...
const getState = (
  state: Partial<FretboardState>,
  next: Partial<FretboardState> = {},
): FretboardState => {
  const display = next.display ?? state.display ?? defaultDisplay
  const rootNote = next.rootNote ?? state.rootNote ?? defaultRootNote
  const scaleName = next.scaleName ?? state.scaleName ?? defaultScaleName
...
function reducer(
  ...
  switch (action.type) {
    ...
    case 'ROOT_NOTE':
      return getState(state, { rootNote: action.payload })
    case 'SCALE_NAME':
      return getState(state, { scaleName: action.payload })
    case 'SELECTED_NOTES': {
      const toggleIndex = action.payload

      return {
        ...state,
        selectedNotes: Array.from(state.selectedNotes).map((selectedNote, i) =>
          i === toggleIndex
...
export function Fretboard(props: FretboardProps) {
  ...
  const [state, dispatch] = React.useReducer(
    ...
    (pState: Partial<FretboardState>) => getState(pState),
  )
